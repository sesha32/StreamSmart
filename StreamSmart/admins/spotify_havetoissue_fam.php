...
$team_id_query = "SELECT MAX(team_id) AS max_team_id FROM spotify_subscriptions";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['team_issue'])) {
    ...
    foreach ($subscription_ids as $subscription_id) {
        ...
        // Update the spotify_subscriptions table
        $update_query = "UPDATE spotify_subscriptions 
        ...
        if ($conn->query($update_query) === TRUE) {
            ...
            $user_query = "SELECT user_id FROM spotify_subscriptions WHERE subscription_id='$subscription_id'";
...
<head>
    ...
    <title>Spotify Subscriptions</title>
    <style>
        ...
        .container {
            width: 80%;
            margin: 20px auto;
            background-color: #282828; /* Spotify dark grey */
            padding: 20px;
            border-radius: 10px;
        ...
        tr:nth-child(odd) {
            background-color: #333; /* Odd row color */
        }
        .delete-icon {
            cursor: pointer;
            color: #1DB954; /* Spotify green */
...
<body>
    ...
    <div class="container">
        <h1>Spotify Subscriptions - Family Plan</h1>
        <?php
        if ($result->num_rows > 0) {
            $count = 0;
            $subscription_ids = [];
            while ($row = $result->fetch_assoc()) {
                if ($count % 6 == 0) { // 6 members per team
                    if ($count > 0) {
                        echo "</tbody></table>
                    ...
                    echo "<div class='team-heading'>Team " . (int)($count / 6 + 1) . "</div>"; // Adjusted to 6 per team
                    echo "<table><thead><tr>
                ...
                echo "<tr>
                        ...
                        <td>
                            <form action='delete_spofamsubscription.php' method='POST' style='display:inline;' onsubmit='event.preventDefault(); confirmDelete(this);'>
                                <input type='hidden' name='subscription_id' value='{$row['subscription_id']}'>
                                <input type='hidden' name='service_type' value='spotify'> <!-- Ensure it's Spotify -->
