...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['team_issue'])) {
    ...
    $plan = $_POST['plan']; // Determine plan type

    foreach ($subscription_ids as $subscription_id) {
        // Calculate the expiration date based on the plan
        if ($plan === 'yearly') {
...
// Fetch subscriptions for the Amazon page (only yearly plans)
...
<body>
    ...
    <div class="container">
        <h1>Amazon Subscriptions - Yearly Plan</h1>
        <?php
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                        ...
                        <td>
                            <form action='delete_amayeasubscription.php' method='POST' style='display:inline;' onsubmit='event.preventDefault(); confirmDelete(this);'>
            ...
            echo "<p>No yearly subscriptions found</p>";
        }
        $conn->close();
        ?>
    </div>
