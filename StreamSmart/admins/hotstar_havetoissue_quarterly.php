...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['team_issue'])) {
    ...
    foreach ($subscription_ids as $subscription_id) {
        // Calculate the expiration date by adding 3 months to the issued_date
        $expire_date = date('Y-m-d H:i:s', strtotime('+3 months', strtotime($issued_date)));
...
$query = "SELECT subscription_id, user_id, first_name, email, mobile, subscription_date 
          ...
          WHERE plan='quarterly' AND issued='no'";
...
<body>
    ...
    <div class="container">
        <h1>Hotstar Subscriptions - Quarterly Plan</h1>
        <?php
        if ($result->num_rows > 0) {
            ...
            while ($row = $result->fetch_assoc()) {
                ...
                echo "<tr>
                        ...
                        <td>
                            <form action='delete_hotquasubscription.php' method='POST' style='display:inline;' onsubmit='event.preventDefault(); confirmDelete(this);'>
