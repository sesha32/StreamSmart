...
$team_id_query = "SELECT MAX(team_id) AS max_team_id FROM youtube_subscriptions";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['team_issue'])) {
    ...
    foreach ($subscription_ids as $subscription_id) {
        ...
        $update_query = "UPDATE youtube_subscriptions 
        ...
        if ($conn->query($update_query) === TRUE) {
            ...
            $user_query = "SELECT user_id FROM youtube_subscriptions WHERE subscription_id='$subscription_id'";
...
// Fetch subscriptions for the YouTube page
...
<head>
    ...
    <style>
        ...
        body {
            ...
            margin: 0;
            padding: 0;
            background-color: #181818; /* YouTube dark gray */
            color: #FFF;
            min-height: 100vh;
            display: flex;
        ...
        .container {
            width: 80%;
            margin: 20px auto;
            background-color: #222; /* YouTube dark gray */
            padding: 20px;
            border-radius: 10px;
        ...
        tr:nth-child(even) {
            background-color: #303030; /* Even row color */
        }
        tr:nth-child(odd) {
            background-color: #383838; /* Odd row color */
        }
        .delete-icon {
            cursor: pointer;
            color: #FF0000; /* YouTube red */
...
<body>
    ...
    <div class="container">
        <h1>YouTube Subscriptions - Monthly Plan</h1>
        <?php
        if ($result->num_rows > 0) {
            $count = 0;
            $subscription_ids = [];
            while ($row = $result->fetch_assoc()) {
                if ($count % 5 == 0) { // 5 members per team
                    if ($count > 0) {
                        echo "</tbody></table>
                    ...
                    echo "<div class='team-heading'>Team " . (int)($count / 5 + 1) . "</div>"; // Adjusted to 5 per team
                    echo "<table><thead><tr>
                ...
                echo "<tr>
                        ...
                        <td>
                            <form action='delete_youmonsubscription.php' method='POST' style='display:inline;' onsubmit='event.preventDefault(); confirmDelete(this);'>
                                <input type='hidden' name='subscription_id' value='{$row['subscription_id']}'>
                                <input type='hidden' name='service_type' value='youtube'> <!-- Ensure it's YouTube -->
